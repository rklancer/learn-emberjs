...
Learn.OutputView = SC.View.extend({
  ...
  didInsertElement: function() {
    ...
    this.$('.output-iframe').load( function() {
      ...
      Learn.outputController.templateChanged();
    });
  }
...
Learn.outputController = SC.Object.create({

  template: "<h1>{{Output.string}}</h1>",

  templateChanged: function() {
    var template = Learn._SC.Handlebars.compile(this.get('template'));
    if (Learn.Output && Learn.Output.view) {
      Learn.Output.view.set('template', template);
