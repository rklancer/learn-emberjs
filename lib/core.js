...
Learn = SC.Application.create({
  ...
  // These properties are observable, but they are set by an observer on outputView to facilitate access by simple dot
  // accessors without needing the get() method (this is useful at the console).
  ...
  outputViewIframeChanged: function() {
    var outputView = this.get('outputView');

    if (!outputView) return;

    this.set('_window',    outputView._window);
    this.set('_document',  outputView._document);
    this.set('_SC',        outputView._SC);
    this.set('_$',         outputView._$);
    this.set('_view',      outputView._view);

  }.observes('.outputView.iframeIsLoaded')
...
Learn.OutputView = SC.View.extend({
  ...
  iframeDidLoad: function(iframeElem) {
    ...
    if (this._updateAfterLoad) {
      this.update();
      this._updateAfterLoad = false;
    }
  ...
  run: function () {
    Learn.set('outputView', this);
    if (this._hasUpdated) {
      this._updateAfterLoad = true;
      this.reloadIframe();
    }
...
Learn.RunButton = SC.Button.extend({
  click: function () {
    ...
    if (outputViewId) SC.View.views[outputViewId].run();
  }
});
